function [Nt_range, koff_values, selectivity, absFlux] = LinearSelAndFluxVsNt()
...
%% User Inputs
Nt_range = logspace(1,4); % sets the range for the x-axis (us^-1 uM^-1)
...
syms TLsym gam koff lam kd Nt real
...
flux_final(koff,Nt) = simplify(subs(subs(subs(subs(subs(subs(...
    flux_3, Ntsym, Nt),TLsym, TL), Lsym, L), D, DF),...
    konsym, kon),koff,koff));

% Find the flux when there's no binding.
flux_no_binding(koff,Nt) = simplify(subs(subs(subs(subs(subs(...
    flux_3, Ntsym, Nt),TLsym, TL), Lsym, L), D, DF),konsym,0));

% Normalize the flux to the non-binding flux.
flux_ratio(koff,Nt) = flux_final(koff,kon)/flux_no_binding;

%Initialize selectivity array
selectivity = nan(length(koff_values),length(Nt_range));

% Plot flux ratio
for kf=koff_values % in uM
y_values = double(flux_ratio(kf,Nt_range));
selectivity(find(koff_values==kf),:) = y_values;
end

SetFigureDefaults(18,2);
figure
semilogx(Nt_range,selectivity,'LineWidth',3);
...
% Plot absolute flux
for kf=koff_values % in uM
y_values = double(flux_final(kf,Nt_range));
absFlux(find(koff_values==kf),:) = y_values;
end

figure
semilogx(Nt_range,absFlux,'LineWidth',3);
h = legend(string(koff_values));
title(h,'$$k_{off}$$ ($$\mu$$s$$^{-1}$$)')
