function [KD_range, DB_values, selectivity] = LinearSelVsKD_NoMech()
...
% non-binding species) as a function of dissociation constant K_D.  No
% mechanism or model of bound diffusion is used here.  Several values of
% bound diffusion coefficent DB can be specified and shown on the plot.
...
% (2) DB_values - a vector containing all DB values used
% (3) selectivity - an array with selectivity results.  First index is DB
% value, second index is K_D.  Plotting selectivity vs K_D will make a plot
% showing all values of DB.

%% User Inputs
KD_range = logspace(-2,3); % sets the range for the x-axis
DB_values = [0.1,0.5,1]; % Bound diffusion constant values to plot
...
syms Dbound TLsym gam koff lam kd real
