function [KD_range, lc_values, selectivity, S_chemEquil] = LinearSelVsKD_LocalChemEquil()
...
% Modified from LinearSelVsKD on 8/28/19 to also assume local chemical
% equilibrium ([complex] = [free TF]*N_t/K_D at all points).  This is to
% show the reviewer that you can't assume local chemical equilibrium (i.e.
% that the results here are different from the results of LinearSelVsKD).

%% User Inputs
KD_range = logspace(-2,3); % sets the range for the x-axis
lc_values = 120; % Nup contour length values to plot (nm)

DF = 0.12; % Free diffusion coefficient, nm^2/us
L = 100; % Length of medium, nm
Nt = 4.7e3; % Total Nup concentration, uM
...
%% Calculate the selectivity in the linear approximation assuming chemical
% equilibrium
DB = DF./(1+3.*DF./(lc_values.*lp.*KD_range.*kon));
S_chemEquil = 1 + DB.*Nt./(DF.*KD_range);
disp('finished local chemical equilibrium calculation');
...
hold on
semilogx(KD_range,S_chemEquil,'LineWidth',3);
hold off
legend({'Linear only','Linear with local chem. eq.'});
xlabel('K_D (\muM)');
ylabel('Selectivity');
